...
After filtering out symmetrically equivalent 3x3 grid designs across all eight rotations and reflections of the square, there are 34,960 unique designs for small urban spaces. An earlier version reported 67,056 because its transformation set accidentally duplicated two operations and omitted the 180-degree rotation and anti-diagonal reflection; consequently, the original sampled pool of 400 layouts may include symmetric duplicates. This number is far too many to run reasonable time mcdp queries without further improvements to the poset structure of the designs. As is, the designs exist in an unordered set ([urban_space.mcdplib/_posets/layout.mcdp_poset](urban_space.mcdplib/_posets/layout.mcdp_poset)), and thus monotonicity is not being leveraged as much as it could be during the queries. I made an attempt to use the budget cost to create an order for the designs, but I was not able to see an obvious improvement to the query times-- this might have been due to some issue on my end.
